...
int DDBlackManager::find(const DDBlackMark& mark)
{
	...
	if (!mark.time.empty())
	{
		...
		for (const auto& [key,id] : multi_range(mTimeIndex, mark.time))
		{
	...
	if (mark.fromGroup.first)
	{
		...
		for (const auto& [key, id] : multi_range(mGroupIndex, mark.fromGroup.first))
		{
	...
	if (mark.fromQQ.first)
	{
		...
		for (const auto& [key, id] : multi_range(mQQIndex, mark.fromQQ.first))
		{
...
{
	int max_danger = 0;
	for (const auto& [group, id] : multi_range(mGroupIndex, llgroup))
	{
...
{
	int max_danger = 0;
	for (const auto& [qq, id] : multi_range(mQQIndex, llqq))
	{
...
void DDBlackManager::rm_black_group(long long llgroup, FromMsg* msg)
{
	...
	for (const auto& [key,index] : multi_range(mGroupIndex, llgroup))
	{
...
void DDBlackManager::rm_black_qq(long long llqq, FromMsg* msg)
{
	...
	for (const auto& [key, index] : multi_range(mQQIndex, llqq))
	{
...
void DDBlackManager::isban(FromMsg* msg)
{
	...
	if (mGroupIndex.count(llID))
	{
		set<int> sRec;
		JsonList jary;
		for (const auto& [key, index] : multi_range(mGroupIndex, llID))
		{
	...
	if (mQQIndex.count(llID))
	{
		set<int> sRec;
		JsonList jary;
		for (const auto& [key, index] : multi_range(mQQIndex, llID))
		{
			jary << vBlackList[index].printJson();
			sRec.insert(index);
		}
...
{
	for (const auto& [group, index] : multi_range(mGroupIndex, llgroup))
	{
...
{
	for (const auto& [qq, index] : multi_range(mQQIndex, llqq))
	{
		const DDBlackMark& mark = vBlackList[index];
...
void DDBlackManager::verify(void* pJson, long long operateQQ)
{
	...
	if (index < 0)
	{
		...
		insert(mark);
		console.log(
			getName(operateQQ) + "已通知" + GlobalMsg["strSelfName"] + "不良记录" + to_string(vBlackList.size() - 1) +
			":\n!warning" + UTF8toGBK(static_cast<json*>(pJson)->dump()), 1, printSTNow());
	}
	else
	{
		//已有记录
		...
		{
			if (credit < 3)return;
			if (credit < 5 && credit < old_mark.danger)return;
		}
		if (update(mark, index, credit))console.log(
			getName(operateQQ) + "已更新" + GlobalMsg["strSelfName"] + "不良记录" + to_string(index) + ":\n!warning" +
			UTF8toGBK(static_cast<json*>(pJson)->dump()), 1, printSTNow());
	}
}
...
{
	json j = freadJson(std::move(strPath));
