...
string js_toUTF8(JSContext* ctx, JSValue val) {
	auto s{ JS_ToCString(ctx, val) };
	string ret{ s };
	JS_FreeCString(ctx, s);
	return ret;
...
string js_Atom2UTF(JSContext* ctx, JSAtom val) {
	auto s{ JS_AtomToCString(ctx, val) };
	string ret{ s };
	JS_FreeCString(ctx, s);
	return ret;
}
JSValue js_newUTF(JSContext* ctx, const string& val) {
	...
	case JS_TAG_OBJECT:
		if (void* p{ nullptr }; JS_GetClassID(val, &p) == js_dice_context_id) {
			...
			if (!JS_GetOwnPropertyNames(ctx, &tab, &len, val, JS_GPN_ENUM_ONLY | JS_GPN_STRING_MASK)) {
				for (uint32_t i = 0; i < len; ++i) {
					obj->set(js_Atom2UTF(ctx, tab[i].atom), js_toAttr(ctx, JS_GetProperty(ctx, val, tab[i].atom)));
				}
...
static JSValue js_NewTable(JSContext* ctx, const ptr<AnysTable>& tab) {
	if (auto t{ tab->getType() }; t == AnysTable::MetaType::Context) {
		...
		for (auto& [key, val] : tab->as_dict()) {
			JS_SetPropertyStr(ctx, dict, key.c_str(), js_newAttr(ctx, val));
		}
		return dict;
	}
	else if (tab->to_list()) {
...
JSValue js_newAttr(JSContext* ctx, const AttrVar& var) {
	...
	case AttrVar::Type::U8String:
		return JS_NewString(ctx, var.text.c_str());
		break;
...
JSValue js_context::evalString(const std::string& s, const string& title) {
	...
	return JS_Eval(ctx, exp.c_str(), exp.length(), title.c_str(), JS_EVAL_TYPE_GLOBAL);
}
JSValue js_context::evalStringLocal(const std::string& exp, const string& title, const ptr<AnysTable>& context) {
	std::lock_guard<std::recursive_mutex> lock(ex_eval);
	return JS_EvalThis(ctx, js_NewTable(ctx, context), exp.c_str(), exp.length(), title.c_str(), JS_EVAL_TYPE_GLOBAL);
}
...
int js_dice_Set_get_own(JSContext* ctx, JSPropertyDescriptor* desc, JSValueConst obj, JSAtom prop) {
	JS2SET(obj);
	if (desc) {
		string key{ js_Atom2UTF(ctx, prop) };
		if (key == "add") {
...
QJSDEF(log) {
	string info{ js_toUTF8(ctx, argv[0]) };
	int note_lv{ 0 };
	int type{ 0 };
...
QJSDEF(getSelfData) {
	string file{ js_toUTF8(ctx, argv[0])};
...
QJSDEF(eventMsg) {
	...
	if (JS_IsObject(argv[0])) {
		...
		string fromMsg{ js_toUTF8(ctx, argv[0]) };
...
QJSDEF(sendMsg) {
	...
	if (JS_IsObject(argv[0])) {
		...
		string msg{ js_toUTF8(ctx, argv[0]) };
...
QJSDEF(getGroupAttr) {
	string item{ js_toUTF8(ctx, argv[1]) };
	...
	if (long long gid{ js_toLongLong(ctx,argv[0]) }) {
		if (item.empty()) return js_newDiceContext(ctx, chat(gid).shared_from_this());
			...
			if (JS_IsUndefined(argv[2])) {
				...
				string subitem{ js_toUTF8(ctx, argv[2]) };
				if (subitem == "card") {
					for (auto uid : DD::getGroupMemberList(gid)) {
						JS_SetPropertyUint32(ctx, items, uid, js_newUTF(ctx, DD::getGroupNick(gid, uid)));
					}
				}
				else if (subitem == "lst") {
					for (auto uid : DD::getGroupMemberList(gid)) {
...
QJSDEF(setGroupAttr) {
	...
	if (item.find("card#") == 0) {
		...
		string card{ js_toUTF8(ctx, argv[2]) };
...
QJSDEF(getPlayerCard) {
	...
	if (uid && PList.count(uid)) {
		...
		if (auto type = JS_VALUE_GET_TAG(argv[1]); type == JS_TAG_STRING) {
			return js_newActor(ctx, pl.getCard(js_toUTF8(ctx, argv[1])));
		}
...
int js_dice_context_get_keys(JSContext* ctx, JSPropertyEnum** ptab, uint32_t* plen, JSValueConst this_val) {
	...
	if ((*plen = obj->size()) > 0) {
		...
		for (const auto& [key, val] : obj->as_dict()) {
			if (auto atom = JS_NewAtom(ctx, key.c_str());
				atom != JS_ATOM_NULL) {
				DD::debugLog("newAtom:" + js_Atom2UTF(ctx, atom) + "#" + to_string(atom));
				tab[i++].atom = atom;
...
int js_dice_context_delete(JSContext* ctx, JSValue this_val, JSAtom atom) {
	JS2OBJ(this_val);
	obj->reset(js_Atom2UTF(ctx, atom));
	return TRUE;
}
...
QJSDEF(context_get) {
	JS2OBJ(this_val);
	if (argc > 0) {
		string key{ js_toUTF8(ctx, argv[0]) };
...
QJSDEF(context_format) {
	JS2OBJ(this_val);
	if (argc > 0) {
		return js_newUTF(ctx, fmt->format(js_toUTF8(ctx, argv[0]), obj));
	}
	else {
...
QJSDEF(GameTable_message) {
	JS2GAME(this_val);
	if (string msg{ js_toUTF8(ctx,argv[0]) }; !msg.empty()) {
...
int js_dice_actor_get_own(JSContext* ctx, JSPropertyDescriptor* desc, JSValueConst this_val, JSAtom prop) {
	JS2PC(this_val);
	if (string key{ js_Atom2UTF(ctx, prop) }; 
		desc && pc
...
int js_dice_actor_get_keys(JSContext* ctx, JSPropertyEnum** ptab, uint32_t* plen, JSValueConst this_val) {
	...
	if ((*plen = pc->size() - 3) > 0) {
		...
		for (const auto& [key, val] : pc->as_dict()) {
			...
			else if (auto atom = JS_NewAtom(ctx, key.c_str());
				atom != JS_ATOM_NULL) {
				tab[i++].atom = atom;
				tab[i++].is_enumerable = TRUE;
			}
...
int js_dice_actor_delete(JSContext* ctx, JSValue this_val, JSAtom atom) {
	JS2PC(this_val);
	string attr{ js_Atom2UTF(ctx, atom) };
...
int js_dice_actor_define(JSContext* ctx, JSValueConst this_obj, JSAtom prop, JSValueConst val, JSValueConst getter, JSValueConst setter, int flags) {
	JS2PC(this_obj);
	switch (pc->set(js_Atom2UTF(ctx, prop), js_toAttr(ctx, val))) {
	case -11:
...
QJSDEF(actor_set) {
	...
	if (JS_IsString(argv[0])) {
		if (0 == pc->set(js_toUTF8(ctx, argv[0]), js_toAttr(ctx, argv[1])))return JS_NewInt32(ctx, 1);
	}
		...
		if (!JS_GetOwnPropertyNames(ctx, &tab, &len, argv[0], JS_GPN_ENUM_ONLY | JS_GPN_STRING_MASK)) {
			for (uint32_t i = 0; i < len; ++i) {
				if (0 == pc->set(js_Atom2UTF(ctx, tab[i].atom), js_toAttr(ctx, JS_GetProperty(ctx, argv[0], tab[i].atom))))++cnt;
			}
			js_free_prop_enum(ctx, tab, len);
		}
...
QJSDEF(actor_rollDice) {
	...
	string exp{ JS_ToBool(ctx,argv[0]) ? js_toUTF8(ctx,argv[0])
	...
	JS_SetPropertyStr(ctx, res, "expr", js_newUTF(ctx, rd.strDice));
...
AttrVar js_context_eval(const std::string& s, const ptr<AnysTable>& context) {
	if (auto ret{ js_main->evalStringLocal(s, "<eval>", context) }; !JS_IsException(ret)) {
		...
		console.log(getMsg("strSelfName") + "执行js语句失败:\n" + s + "\n" + js_main->getException() , 0b10);
	}
	return {};
}
...
bool js_call_event(const ptr<AnysTable>& eve, const AttrVar& action) {
	...
	if (auto ret = isFile ? js_event_pool[title].evalFileLocal(fmt->js_path(script), eve) 
		...
		JS_IsException(ret)) {
		console.log(getMsg("strSelfName") + "事件「" + title + "」执行js失败!\n" + js_event_pool[title].getException(), 0b10);
		return false;
	}
	return true;
}
void js_msg_call(DiceEvent* msg, const AttrVar& js) {
	...
	if (fmt->has_js(jScript)) {
		...
		if (JS_IsException(ret = ctx.evalString(jScript, msg->get_str("reply_title")))) {
			console.log(getMsg("strSelfName") + "响应「" + msg->get_str("reply_title") + "」js语句失败!\n" + ctx.getException(), 0b10);
			msg->set("lang", "JavaScript");
			msg->reply(getMsg("strScriptRunErr"));
		}
		else if (auto ans{ js_toAttr(ctx, ret) }; ans.is_character()) {
...
AttrVar js_eval_simple(const std::string& exp) {
	...
	if (JSValue ret{ ctx.evalString(exp, "simple formula") }; !JS_IsException(ret)) {
		...
		console.log(getMsg("strSelfName") + "计算js语句" + exp + "失败!\n" + ctx.getException(), 0b10);
		return {};
	}
}