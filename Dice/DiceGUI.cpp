...
public:
    static LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        {
            pThis = (T*)GetWindowLongPtrA(hwnd, GWLP_USERDATA);
        }
    ...
    )
    {
        ...
        wc.hIcon = (HICON)LoadImageA(hDllModule, MAKEINTRESOURCEA(IDI_ICON), IMAGE_ICON, 256, 256, LR_LOADTRANSPARENT | LR_SHARED);
...
int CALLBACK ListViewUserTrustComp(LPARAM lp1, LPARAM lp2, LPARAM sortParam)
{
	...
	try
	{
        ...
        {
        case 0:
            if (isAsc) return (int)std::clamp(std::stoll(str1) - std::stoll(str2), (long long)INT_MIN , (long long)INT_MAX);
            else return (int)std::clamp(std::stoll(str2) - std::stoll(str1), (long long)INT_MIN, (long long)INT_MAX);
                break;
        case 1:
            return 0;
        case 2:
    ...
    case WM_CTLCOLORSTATIC:
	    {
	        ...
	        return (INT_PTR)(HBRUSH)GetStockObject(COLOR_WINDOW + 1);
	    }
    case WM_COMMAND:
	    {
            ...
            case ID_MASTER_BUTTONUSERTRUSTREMOVE:
	            {
	                ...
	                {
                        if (ret != -1) ListView_DeleteItem(ListViewUserTrust.Window(), ret);
            ...
            case ID_ABOUT_BUTTONQQGROUP:
	            {
					ShellExecute(m_hwnd, "open", "https://jq.qq.com/?_wv=1027&k=Wy9CsHcq", nullptr, nullptr, SW_SHOWDEFAULT);
	            }
                return 0;
    ...
    case WM_NOTIFY:
	    {
            ...
            case LVN_COLUMNCLICK:
                if (((LPNMHDR)lParam)->idFrom == ID_MASTER_LISTVIEWUSERTRUST)
                {
                    ...
                    ListViewSorting sort{ &this->ListViewUserTrust, nSortColumn, (bool)bSortAscending };
                    ListView_SortItemsEx(ListViewUserTrust.Window(), ListViewUserTrustComp, reinterpret_cast<LPARAM>(&sort));
                }
...
LRESULT DiceGUI::CreateCustomMsgPage()
{
    ...
    EditCustomMsg.Create("请双击列表中的项目", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL | WS_BORDER, 0,
    ...
    ListViewCustomMsg.Create("",
        ...
        m_hwnd,
        (HMENU)IDM_LIST_MSG);
    ListViewCustomMsg.SetExtendedListViewStyle(LVS_EX_DOUBLEBUFFER | LVS_EX_AUTOSIZECOLUMNS | LVS_EX_TWOCLICKACTIVATE | LVS_EX_UNDERLINEHOT);
...
LRESULT DiceGUI::CreateMasterPage()
{
    ...
    StaticMasterLabel.Create((!console ? "Master模式已关闭" : ("当前的Master为" + to_string(console.masterQQ) + "(设置QQ为0以关闭Master模式)").c_str()),
    ...
    ListViewUserTrust.SetExtendedListViewStyle(LVS_EX_DOUBLEBUFFER | LVS_EX_AUTOSIZECOLUMNS | LVS_EX_TWOCLICKACTIVATE | LVS_EX_UNDERLINEHOT);
    ...
    ListViewConfig.SetExtendedListViewStyle(LVS_EX_DOUBLEBUFFER | LVS_EX_AUTOSIZECOLUMNS | LVS_EX_TWOCLICKACTIVATE | LVS_EX_UNDERLINEHOT);
...
LRESULT DiceGUI::CreateAboutPage()
{
    ...
    HBITMAP hBitmap = (HBITMAP)LoadImageA(hDllModule, MAKEINTRESOURCEA(ID_BITMAP_DICELOGO), IMAGE_BITMAP, 0, 0, LR_SHARED);
...
int WINAPI GUIMain()
{
	...
	// 进度条
    HWND progress = CreateWindowA(PROGRESS_CLASSA, nullptr, WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_BORDER | PBS_SMOOTHREVERSE | PBS_SMOOTH
	...
	if (!MainWindow.Create("Dice! GUI", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_BORDER | WS_CLIPSIBLINGS, 0, CW_USEDEFAULT, CW_USEDEFAULT,1146, 564))
	{
        return 0;
	}
