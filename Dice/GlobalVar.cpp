...
std::map<const std::string, std::string> GlobalMsg
{
	...
	{"strRequestRetCodeErr", "访问服务器时出现错误! HTTP状态码: {http_status_code}"},
	...
	{"strProp", "{nick}的{prop_name}属性值为{prop_value}"},
	{"strStErr", "格式错误:请参考帮助文档获取.st命令的使用方法"},
	{"strRulesFormatErr", "格式错误:正确格式为.rules[规则名称:]规则条目 如.rules COC7:力量"},
	{"strJrrp", "{nick}今天的人品值是: {jrrp_value}"},
	{"strJrrpErr", "JRRP获取失败! 错误信息: \n{error_msg}"},
	{"strInitReplyMsg", "{nick}的先攻骰点: {dice_res}"},
	{"strHiddenGroupPrefix", "在群\"{group_name}\"中 "},
	{"strHiddenDiscussPrefix", "在多人聊天中 "},
	{"strTurnNoticeReplyMsg", "{nick}的掷骰轮数: {dice_res}轮"},
	{"strHiddenDiceReplyMsg", "{nick}进行了一次暗骰"},
	{"strRollDiceReplyMsg", "{nick}骰出了: {dice_res}"},
	{"strRollDiceReasonPrefix", "由于{reason} "},
	{"strObListReplyMsg", "当前的旁观者有: {ob_list}"},
	{"strObListEmptyNotice", "当前暂无旁观者"},
	{"strNickChangeReplyMsg", "已将{nick}的{is_global}名称更改为{new_nick}"},
	{"strNickDeleteReplyMsg", "已将{nick}的{is_global}名称删除"},
	{"strFumble", "大失败"},
	{"strFailure", "失败"},
	{"strSuccess", "成功"},
	{"strHardSuccess", "困难成功"},
